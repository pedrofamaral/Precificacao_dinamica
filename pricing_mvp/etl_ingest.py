...
# --- bootstrap: permite rodar como módulo (-m) ou script direto ---
...
try:
    ...
    # caso esteja rodando como script direto
    from pricing_mvp.common import (
        SETTINGS, logger, ensure_dirs, iter_files, load_json_lines, norm_sku,
        to_sql, read_sql, exec_sql, seen, mark_seen, file_fingerprint
    )
...
def to_float(x):
    ...
    if isinstance(x, (int, float)):
        return float(x)
    s = str(x).strip()
    # remove símbolos monetários e normaliza separadores
    s = s.replace("R$", "").replace("$", "").replace("€", "").strip()
    # primeiro tira separador de milhar (.)
    s = s.replace(".", "")
    # depois troca vírgula por ponto
    s = s.replace(",", ".")
    try:
        return float(s)
    except ValueError:
...
def unwrap_ml_click(url: str | None) -> str | None:
    """Tenta extrair a URL real quando for link de redirecionamento do Mercado Livre.
    Nem todos os links trazem 'url=' na query; quando não houver, volta o original."""
...
def name_from_url(url: str | None) -> str | None:
    """Extrai um nome legível do slug da URL (com fallback para None se genérica)."""
    if not url or not isinstance(url, str):
        return None
    try:
        ...
        if not seg:
            return None
        last = seg[-1]
        # Se o último segmento for um ID ML (MLB...), usa o anterior quando possível
...
def name_from_query(query: str | None) -> str | None:
    ...
    if not query or not isinstance(query, str):
        return None
    # remove sufixo de timestamp + extensão
...
def parse_captured_from_query(q: str | None):
    """Extrai timestamp do padrão _YYYYMMDD_HHMMSS no nome de arquivo."""
...
def ingest_sqlite() -> List[Dict[str, Any]]:
    """Lê .db em SETTINGS.sqlite_dir. Tenta detectar uma tabela compatível."""
    rows = []
...
def clean_and_snapshot(all_rows_df: pd.DataFrame):
    ...
    # garantir que price é numérico (para dados legados)
    ...
    # 6) Nome canônico por (marketplace, sku_norm)
    ...
    # 8) Diagnóstico de descarte (corrigido)
    try:
...
def main():
    ...
    if not all_rows:
        ...
        # Mesmo sem dados novos, reconstroi o snapshot limpo a partir do banco atual:
        dummy = pd.DataFrame()
        clean_and_snapshot(dummy)
        return

    # DataFrame bruto dos novos registros
    df_new = pd.DataFrame(all_rows)

    # Tipagem básica dos novos registros
